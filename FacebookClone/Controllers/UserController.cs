...
namespace FacebookClone.Controllers
{
	...
	public class UserController : BaseController
	{
		...
		[Authorize]
		public async Task<IActionResult> Activation(ActivationDTO activationDTO)
		{
			return await HandleRequest(async () =>
			{
				var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
				await _userRepository.SetUserActivationAsync(Guid.Parse(userId), activationDTO.IsActive);
				return Ok();
			});
